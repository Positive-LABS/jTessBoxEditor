...
public class GuiWithEdit extends GuiWithMRU {
    ...
    void DeleteLineActionPerformed(java.awt.event.ActionEvent evt)
    {
        ...
        List<TessBox> selected = boxes.getSelectedBoxes();
//        List<TessBox> selectedToRemove = boxes.getSelectedBoxes();
        if (selected.size() <= 0) {
            JOptionPane.showMessageDialog(this, "Please select a box or more to delete.");
            return;
        }
        List<Integer> idxArr = new ArrayList<Integer>();
        List<TessBox> listboxes = this.boxes.toList();
        for (TessBox box : selected) {
            int listsboxesSize = listboxes.size();
            int index = listboxes.indexOf(box);
            short pg = box.getPage();
            Rectangle rect = box.getRect();
            if (selected.size() == 1) {
                int idx = index;
                TessBox tmpbox = listboxes.get(idx);
                Rectangle tmprect = tmpbox.getRect();
                short tmppg = tmpbox.getPage();
                while(abs( tmprect.y - rect.y) < 4 && tmppg == pg && idx > 0){
//                    selectedToRemove.add(tmpbox);
                    idxArr.add(idx);
                    idx --;
                    if ( idx > 0) {
                        tmpbox = listboxes.get(idx);
                        tmprect = tmpbox.getRect();
                        tmppg = tmpbox.getPage();
                    }
                }
                idx = index+1;
                tmpbox = listboxes.get(idx);
                tmprect = tmpbox.getRect();
                tmppg = tmpbox.getPage();
                while(abs( tmprect.y - rect.y) <4 && tmppg == pg && idx < listsboxesSize){
//                    selectedToRemove.add(tmpbox);
                    idxArr.add(idx);
                    idx ++;
                    if ( idx < listsboxesSize) {
                        tmpbox = listboxes.get(idx);
                        tmprect = tmpbox.getRect();
                        tmppg = tmpbox.getPage();
                    }
        ...
        Collections.sort(idxArr, Collections.reverseOrder());
        for (int index : idxArr) {
            TessBox box = listboxes.get(index);
            this.boxes.remove(box);
